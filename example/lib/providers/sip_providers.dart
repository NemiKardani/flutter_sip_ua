...
SipAccount? readPersistedAccount(SharedPreferences prefs) {
  ...
  final serverUri = Uri.parse(parts[0]);
  SipTransportType transportType;
  if (parts.length > 7) {
    transportType = SipTransportType.values.firstWhere(
      (e) => e.name == parts[7].toLowerCase(),
      orElse: () => _inferTransportType(serverUri),
    );
  } else {
    transportType = _inferTransportType(serverUri);
  }
  return SipAccount(
    serverUri: serverUri,
    domain: parts[1],
...
SipTransportType _inferTransportType(Uri uri) {
  final scheme = uri.scheme.toLowerCase();
  if (scheme == 'ws') return SipTransportType.ws;
  if (scheme == 'wss') return SipTransportType.wss;
  if (scheme == 'sips') return SipTransportType.tls;
  return SipTransportType.udp;
}
